...
def write_types(f):
    ...
    for t in dtypes:
        ...
        f.write(f"    real({t:s}) :: value"+newline)
        f.write(f"    real({t:s}) :: uncertainty"+newline)
...
def write_constant(f, var, name, value, uncertainty, unit):
    ...
    for t in dtypes:
        ...
        if len(dtypes) <= 1:
            ...
            suffix = f"_{t:s}"
        f.write(f"type(codata_constant_type"+suffix+"), parameter, public :: "+\
                f"{var}"+suffix+" = &"+newline +\
                f"codata_constant_type"+suffix+f"(\"{name:s}\", &"+newline+\
