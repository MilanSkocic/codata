...
subroutine write_fortran_module_declaration(fcode, year)
    ...
    write(fcode, "(A)") "use iso_fortran_env, only: dp=>real64, int32"
    write(fcode, "(A)") "use iso_c_binding, only: cdp=>c_double, c_int"
...
subroutine write_all_constants(fcodata, ffortran, fcheader, fcpython, props)
    ...
    do i=1, props%n
        ...
        if(len(line)>0)then
            ...
            write(ffortran, "(A)") 'real(dp), parameter, public :: '&
            //trim(name)//suffix//'='//trim(value)//'_dp'//' !< '//trim(unit)
            write(ffortran, "(A,/,A)") 'real(dp), parameter, public :: '//&
            'U_'//trim(name)//suffix//'='//trim(uncertainty)//'_dp'//' !< '//trim(unit)
            write(ffortran, "(A,/,A)") 'real(cdp), protected, public, bind(C,name="'//trim(name)//suffix//'"):: &', &
            'capi_'//trim(capi_dummy)//suffix//'='//trim(name)//suffix//' !< C API '//trim(unit)
            write(ffortran, "(A,/,A)") 'real(cdp), protected, public, bind(C,name="U_'//trim(name)//suffix//'") :: &', &
